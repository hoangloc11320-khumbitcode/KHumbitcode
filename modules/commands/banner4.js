module.exports.config = {
  ...
  hasPermssion: 0,
  credits: "Raiku",
  description: "Tạo ảnh đẹp",
  commandCategory: "Tạo ảnh",
  usages: "banner4 + ID nhân vật | Tên | Chữ Ký | Facebook",
  cooldowns: 5
};
...
const text1 = args.join(" ").trim().replace(/\s+/g, " ").replace(/(\s+\|)/g, "|").replace(/\|\s+/g, "|").split("|")[0] || "TEXT",
  ...
  let pathImg = __dirname + `/font/avatar_1.png`;
  ...
  if (!fs.existsSync(__dirname +
    ...
    let getfont = (await axios.get(`https://github.com/J-JRT/font/raw/mainV2/Catamaran-Regular.ttf`, { responseType: "arraybuffer" })).data;
    fs.writeFileSync(__dirname + `/font/Catamaran-Regular.ttf`, Buffer.from(getfont, "utf-8"));
  };
  if (!fs.existsSync(__dirname +
    `/font/Metropolis-Black.ttf`)) {
    let getfont1 = (await axios.get(`https://github.com/J-JRT/font/raw/mainV2/Metropolis-Black.ttf`, { responseType: "arraybuffer" })).data;
    fs.writeFileSync(__dirname + `/font/Metropolis-Black.ttf`, Buffer.from(getfont1, "utf-8"));
  };
  if (!fs.existsSync(__dirname +
    `/font/UTMAlterGothic.ttf`)) {
    let getfont1 = (await axios.get(`https://github.com/J-JRT/font/raw/mainV2/UTMAlterGothic.ttf`, { responseType: "arraybuffer" })).data;
    fs.writeFileSync(__dirname + `/font/UTMAlterGothic.ttf`, Buffer.from(getfont1, "utf-8"));
  }
...
Canvas.registerFont(__dirname + `/font/Metropolis-Black.ttf`, {
...
ctx.drawImage(canvas2 , 0, 0, canvas.width, canvas.height)
   Canvas.registerFont(__dirname + `/font/Catamaran-Regular.ttf`, {
  ...
  ctx.drawImage(canvas3 , 0, 0, canvas.width, canvas.height)
   Canvas.registerFont(__dirname + `/font/UTMAlterGothic.ttf`, {
